...
int linear::HashTable::primes_[PRIME_SIZE] = {3, 7, 23, 47, 97, 251, 443, 911, 1471, 42773};

linear::HashTable::HashTable(int size, double loadFactor)
    : useBucketNum_(0)
...
linear::HashTable::~HashTable() {
    delete[] table_;
    table_ = nullptr;
}

double linear::HashTable::getFactor() const {
    double factor = useBucketNum_ * 1.0 / tableSize_;
    std::cout << "factor: " << factor << '\n';
    return factor;
}

bool linear::HashTable::insert(int key) {
    if (getFactor() > loadFactor_) {
        expand();
    }
...
bool linear::HashTable::erase(int key) {
    int idx = key % tableSize_;
    if (idx < 0)
        idx += tableSize_;
    int i = idx;
...
bool linear::HashTable::find(int key) const {
    int idx = key % tableSize_;
    if (idx < 0)
        idx += tableSize_;
    int i = idx;
...
int linked::HashTable::primes_[PRIME_SIZE] = {3, 7, 23, 47, 97, 251, 443, 911, 1471, 42773};

linked::HashTable::HashTable(int size, double loadFactor)
    : useBucketNum_(0)
    , loadFactor_(loadFactor)
    , primeIdx_(0)
{
...
double linked::HashTable::getFactor() const {
    double factor = useBucketNum_ * 1.0 / table_.size();
    std::cout << "factor: " << factor << '\n';
    return factor;
}

void linked::HashTable::insert(int key) {
    if (getFactor() > loadFactor_)
        expand();
...
void linked::HashTable::erase(int key) {
    int idx = key % table_.size();
    if (idx < 0)
        idx += table_.size();
    auto it = std::find(table_[idx].begin(), table_[idx].end(), key);
...
bool linked::HashTable::find(int key) const {
    int idx = key % table_.size();
    if (idx < 0)
        idx += table_.size();
    auto it = std::find(table_[idx].begin(), table_[idx].end(), key);
